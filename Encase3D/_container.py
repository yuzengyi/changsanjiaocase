...
class Container(object):
    ...
    def _encase(self, cargo: Cargo) -> Point:
        ...
        for point in self._available_points:
            if (
                    ...
                    point.x + cargo.length < self._horizontal_planar and  # 判断横坐标是否超出边界
        ...
        if not flag.is_valid:
            if (
                ...
                elif self._vertical_planar < self.height:  # 如果不能在边界上放置，就向上升高参考面
    ...
    def yanzhan_encase(self, cargo: Cargo) -> Point:
        ...
        for point in self._available_points:
            if (
                    ...
                    point.x + cargo.length < self._horizontal_planar*1.05 and  # 判断横坐标是否超出边界
                    point.z + cargo.height < self._vertical_planar*1.05  # 判断纵坐标是否超出边界
            ):
                flag = point
                break  # 找到一个符合要求的点就将其标记为待放置点并中断循环
        # 如果找不到可用点位，则先检查当前参考面是否位于横轴边界上
    ...
    def daiziis_encasable(self, site: Point, cargo: Cargo) -> bool:
        ...
        if self.maxy <= temp.y + temp.width:
            self.maxy = temp.y + temp.width
        if (
            self.maxx+temp.height+temp.z > self.length+1 or ## 袋子长+袋子高 >= 物品长+物品高
    ...
    def yanzhandaiziis_encasable(self, site: Point, cargo: Cargo) -> bool:
        ...
        if self.maxz <= temp.z + temp.height:
            self.maxz = temp.z + temp.height
        if (
            self.maxx+temp.height+temp.z > (self.length+1)*1.05 or ## 袋子长+袋子高 >= 物品长+物品高
            self.maxy + temp.height+temp.z> (self.width + 1)*1.05 ## 袋子宽+袋子高 >= 物品宽+物品高
        ):
            encasable = False
        for setted_cargo in self._setted_cargos:
            if _is_cargos_collide(temp, setted_cargo):
    ...
    def daizi_encase(self, cargo: Cargo) -> Point:
        ...
        for point in self._available_points:
            if (
                    ...
                    point.x + cargo.length + cargo.height< self._horizontal_planar +1 and  # 判断横坐标是否超出边界
                    point.z + cargo.width + cargo.height < self._vertical_planar+1  # 判断纵坐标是否超出边界
            ):
                flag = point
                break  # 找到一个符合要求的点就将其标记为待放置点并中断循环
        # 如果找不到可用点位，则先检查当前参考面是否位于横轴边界上
